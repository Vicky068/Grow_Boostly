(function ($) {
	...
	const menuWrap = document.querySelector('header.style-4 .menu-wrap');
	const mobileMenuQuery = window.matchMedia('(max-width: 991px)');

	function placeMobileMenu() {
		if (!mainMenu || !menuWrap) {
			return;
		}
		const navRight = menuWrap.querySelector('.nav-right');
		if (mobileMenuQuery.matches) {
			if (mainMenu.parentElement !== document.body) {
				document.body.appendChild(mainMenu);
			}
			mainMenu.classList.add('gb-mobile-nav');
			return;
		}
		mainMenu.classList.remove('gb-mobile-nav');
		if (mainMenu.parentElement !== menuWrap) {
			menuWrap.insertBefore(mainMenu, navRight);
		}
	}

	function initMobileMenuHost() {
		placeMobileMenu();
	}

	if (document.readyState === 'loading') {
		document.addEventListener('DOMContentLoaded', initMobileMenuHost);
	} else {
		initMobileMenuHost();
	}

	if (typeof mobileMenuQuery.addEventListener === 'function') {
		mobileMenuQuery.addEventListener('change', placeMobileMenu);
	} else {
		mobileMenuQuery.addListener(placeMobileMenu);
	}

	function collapseMobileSubmenus() {
		jQuery('.main-menu .menu-list > li.submenu-open').removeClass('submenu-open');
		jQuery('.main-menu .dropdown-icon').removeClass('active');
	}
	...
	function toggleMobileMenu() {
		...
		const isOpen = !mainMenu.classList.contains('show-menu');
		if (isOpen) {
			collapseMobileSubmenus();
	...
	if (sidebarButton) {
		sidebarButton.addEventListener('click', toggleMobileMenu);
	}
	...
	function toggleMobileSubmenu($trigger) {
		...
		var $parent = $trigger.closest('li.menu-item-has-children');
		if (!$parent.length) {
			return false;
		}
		var $icon = $trigger.hasClass('dropdown-icon') ? $trigger : $trigger.siblings('.dropdown-icon');
		if (!$icon.length) {
			return false;
		}
		var isOpen = $parent.hasClass('submenu-open');
		$parent.siblings('.submenu-open').removeClass('submenu-open').find('.dropdown-icon').removeClass('active');
		$parent.toggleClass('submenu-open', !isOpen);
		$icon.toggleClass('active', !isOpen);
		return true;
	}

	jQuery(document).on('click', '.main-menu .dropdown-icon', function (e) {
		e.preventDefault();
		e.stopPropagation();
		if (toggleMobileSubmenu(jQuery(this))) {
			return;
		}
		var $parent = jQuery(this).parent();
		var $submenu = jQuery(this).next('ul, .mega-menu, .mega-menu2, .marketing-mega-menu, .development-mega-menu');
		jQuery(this).toggleClass('active');
		$parent.toggleClass('active');
		$submenu.slideToggle();
		$parent.siblings().children('ul, .mega-menu, .mega-menu2, .marketing-mega-menu, .development-mega-menu').slideUp();
		$parent.siblings().removeClass('active').children('.active').removeClass('active');
	});

	jQuery(document).on('click', '.main-menu .menu-item-has-children > a.drop-down', function (e) {
		if (window.innerWidth > 991) {
			return;
		}
		e.preventDefault();
